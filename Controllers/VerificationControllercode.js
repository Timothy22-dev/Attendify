const Attendance = require('../models/'); // Adjust the path to your Attendance model
...
exports.verifyAttendance = async (req, res) => {
    ...
    try {
        ...
        const student = await User.findOne({ matriculationNumber });
        if (!student) {
            return res.status(404).json({ message: 'Student not found' });
        }

        // 2. Get the saved code for this student
        const savedCode = await Code.findOne({ user: student._id });
        if (!savedCode) {
